...
class CaloriesComplicationService : SuspendingComplicationDataSourceService() {
    ...
    override suspend fun onComplicationRequest(request: ComplicationRequest): ComplicationData? {
        ...
        val desc = PlainComplicationText.Builder("Calories").build()
        val tapAction = ComplicationIntents.healthTapIntent(this)

        return when (request.complicationType) {
            ComplicationType.RANGED_VALUE -> {
                RangedValueComplicationData.Builder(
                    value = ComplicationValueSanitizer.sanitize(numValue, min = 0f, max = 3000.0f),
                    min = 0f,
                    max = 3000.0f,
    ...
    override fun getPreviewData(type: ComplicationType): ComplicationData? {
        ...
        val text = PlainComplicationText.Builder("1500").build()
        val desc = PlainComplicationText.Builder("Calories").build()
        val tapAction = ComplicationIntents.healthTapIntent(this)
